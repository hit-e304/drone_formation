...
{
    UAV1_local_tar_pose = *local_tar_pos2;
}
...
int main(int argc, char **argv)
{
    ...
    // double distance_UAV1_UAV3 = sqrt(pow(UAV1_local_pose.pose.position.x - UAV3_local_pose.pose.position.x, 2) + pow(UAV1_local_pose.pose.position.y - UAV3_local_pose.pose.position.y, 2) + pow(UAV1_local_pose.pose.position.z - UAV3_local_pose.pose.position.z, 2));
    ...
    while(ros::ok())
    {
        ...
        else if(UAV2_current_mission_state == TrackingUAV1)
        {
            ...
            {
                v23_x = k_v / (current_pose2.pose.position.x - UAV3_local_pose.pose.position.x);
                v23_y = k_v / (current_pose2.pose.position.y - UAV3_local_pose.pose.position.y);
            }
            else
            {
                v23_x = 0.0;
            ...
            position_target_local2.velocity.x = v12_x + v23_x;
            position_target_local2.velocity.y = v12_y + v23_y;
        }
        UAV2_target_position_pub.publish(position_target_local2);
        ros::spinOnce();
        rate.sleep();
